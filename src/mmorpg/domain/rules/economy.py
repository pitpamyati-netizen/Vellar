...
#: уровень: репутация держит лавку открытой чуть шире.
...
#: сырьё не взять вовсе (ADR 0056), поэтому его наличие не бросают из сида:
#: сточенную кирку меняют в любом городе и в любой переворот. Дороже этих двух
#: инструмент бывает - но уже удачей общего прилавка.
TOOL_RARITIES: tuple[str, ...] = ("common", "uncommon")

# Престол берёт сбор с каждой сделки между игроками (``Narrative.md``, раздел 9).
# Это единственный отток золота, который растёт вместе с тем, сколько игроки на
# самом деле торгуют.
TRADE_TAX_PERCENT = 5
...
def roll_assortment(
    ...
    *,
    ...
    Чем выше репутация, тем шире прилавок: ``reputation`` приходит процентами
    ``reputation_percent`` со всего, что надето и выбрано (``REPUTATION_PER_STEP``).
    ``strain`` (0…1, ``mood.city_strain``, ADR 0055) - «нужда» города по состоянию
    округи: сужает прилавок, но не ниже ``STOCK_MIN``.

    Редкость решает, что вообще может лечь на прилавок: без веса лавка
...
def trade_tax(price: int, *, percent: int = TRADE_TAX_PERCENT) -> int:
    ...
    Берётся один раз, с цены, и уходит из игры целиком. Передача даром
    (``передать``) сделкой не считается и не облагается: облагать подарок значило
    бы наказывать игроков за помощь друг другу. Сделка с ценой всегда стоит хотя
    бы одну золотую, поэтому ливень продаж по монете не проведёт ценность мимо
    пошлины.
    """
    if price <= 0:
        return 0
...
def refund(price: int, tax: int) -> int:
    ...
    Ровно то, что было выдано продавцу, и ни монетой больше. Пошлина ушла из игры
    в момент расчёта, и возврат полной цены напечатал бы её из ничего
    (``Claude.md``, правило 8).
    """
...
# ночь стоит трёх схваток и на пятом уровне, и на сто пятидесятом.
...
# Дорога до другого города: платят за наёмную повозку и охрану (``Narrative.md``,
# Дом Порубежья). Цена растёт и с уровнем, и с числом городов между тем, где
# стоишь, и тем, куда идёшь. Таймеров нет - дорога проходится сразу (ADR 0051).
...
# (ADR 0057). Доля, а не своя кривая, - цена вещи уже считает и ступень, и
# редкость. Меньше единицы с запасом: починка, стоящая как новая вещь, - это
# не починка, а лавка.
REPAIR_SHARE = 0.15
