...
# Поражение дорого, но никогда не разорительно: десятая часть того, что при
# тебе, и пробуждение в городе с четвертью здоровья.
DEFEAT_GOLD_PERCENT = 10
DEFEAT_HEALTH_PERCENT = 25

# Тихий узел платит долю от того, что платит бой того же уровня: поиск -
# безопасный способ потратить стражу, а значит, и более медленный. Доля, а не
# своя кривая (ADR 0058): всё, что игра платит золотом, меряется одним боем
# своего уровня (``procgen/enemies.gold_at``).
...
#: Прибавка к тому, что приносит тихий узел: тайник, находка, развилка.
#: Святилище он не трогает: оно лечит, а не платит.
EVENT_REWARD_KEY = "event_reward_percent"

#: Что лежит в узле для сбора, по его имени - и каким инструментом это берут
#: (ADR 0056). Из зарослей не выкапывают руду, а с останков не срезают травы.
#: Имена приходят из постоянного имени узла
#: (``procgen/location._CATEGORY_NAMES``, категория «находка»); узел, которого
#: тут нет, отдаёт любое сырьё по уровню. Имя у узла постоянно, а вид - сбор он
#: или тайник - меняется с поколением, поэтому здесь только те имена, что
#: честны при любом из видов.
...
def carry_wounds(
    ...
    Ниже единицы оно не опускается: персонаж с нулём здоровья не может ни
    драться, ни дойти до лекаря, и это был бы тупик, а не цена поражения.
    """
...
def _gather(
    ...
    *,
    ...
    руках. Сколько вышло, решает ранг ремесла, а стоит это одной единицы
    прочности - всегда, какой бы богатой ни была жила.
    """
    if tool is None:
        return Gathered(character=character)
    ...
    # Ступень сырья решает **место**, а не тот, кто в него пришёл: мастер,
    ...
    if not here:
        # Земля, в которой этому ремеслу нечего взять, не остаётся немой: жила отдаёт
        # то, что вообще бывает такого рода, - иначе игрок стачивал бы инструмент о
        # пустое место, ничего об этом не зная.
...
# --- дно спуска -------------------------------------------------------
#
# Награда за заход, пройденный до логова: примерно ещё один эпический бой -
# столько, чтобы спускаться раненым было решением, а уйти на втором этаже
# чего-то стоило.
...
def descent_prize(
    ...
    *,
    ...
    Находка никогда не бывает сырьём: дно ямы - это место, где лежит вещь, а не
    место, где растут травы.
...
def rest(content: GameContent, character: Character, *, paid: bool) -> RestResult:
    ...
    Бесплатная постель существует затем, чтобы разорившийся персонаж не застревал
    без единого хода.
    """
    stats = derived_stats(content, character)
...
def _pick_item(content: GameContent, source: random.Random, level: int) -> str:
    ...
    Сырьё сюда не попадает: его не находят - его собирают, и только инструментом
    (``_gather``, ADR 0056). Тайник платит тем, что можно надеть, выпить или
    продать.
    """
    pool = [
        item
