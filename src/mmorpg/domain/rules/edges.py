...
Грань выбирают один раз, на третьем ранге, и она — единственная развилка умения
(``docs/skills.md``). Долго она была развилкой только на словах: в содержимом обе
грани каждого из 128 умений описаны своим действием («цель теряет здоровье ещё
три хода», «задевает вторую цель на 60 процентов урона»), а движок делал для
любой первой грани одно и то же — плюс двадцать процентов силы — и для любой
второй тоже одно — минус тридцать пять процентов стоимости. Двести пятьдесят
шесть обещаний, ни одного выполненного.

Здесь описан словарь, которым грань говорит о себе в ``skills.toml``, и правило,
по которому этот словарь ложится на умение. Словарь маленький нарочно: грань —
это поправка к умению, а не второе умение, и всё, что она умеет, — сдвинуть одно
из чисел, которыми умение уже описано (``skill_effects.EffectSpec``). Новая грань
не требует кода; требует его только новый **род** поправки, и тогда это одно поле
здесь и одна строка в ``combat``.
