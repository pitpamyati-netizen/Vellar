...
Круг арены дерётся против персонажа другого игрока, **которым играет движок**:
тот же боец, какого движок собирает кому угодно, только его нажатий никто не
ждёт (ADR 0021). Дерётся он своим оружием и своими умениями; его не вызывают,
не торопят, и теряет он ничего - ставка стоит между каждым бойцом и собственным
кошельком арены. Таймеров нет (``docs/accessibility.md``, правило 13).

- ставка растёт с уровнем, поэтому на сотом она стоит того же, что на десятом;
...
**Арена не печатает золото**: она платит из долга, который держит, - из ставок,
взятых с этого персонажа и ещё не возвращённых, - поэтому за всю жизнь никто не
заберёт из неё больше, чем в неё положил. Выигрывают там счёт, а не доход.

Новичку первый круг выдают в долг (``WELCOME_ROUNDS``): первая победа, платящая
ровно ставку, читается ошибкой, а не правилом.
"""
