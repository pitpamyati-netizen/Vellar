...
1. **У оружия есть кости.** Урон - не доля чего-то, а число в границах: «от 2
   до 124». Кости берутся у рода (``WeaponType.dice``) и растут гранями со
   ступенью вещи (``domain/procgen/items.py``). Голыми руками бьют тоже
   костями, только маленькими.
...
4. **Чужая вещь не запрещена - она дорога.** Класс носит своё
   (``classes.toml``), но надеть можно что угодно: за не своё оружие и не свой
   доспех платят точностью и инициативой (ADR 0064). Запрет молчит, а штраф
   говорит, и говорит числом.

Умение - отдельный разговор: оно может просить род оружия (``skills.toml``), и
без него не срабатывает вовсе. Выстрел без лука - это не «хуже», это «нечем».
...
# линейно, и несмягчённая броня выиграла бы гонку - к концу полосы обычный удар
# доходил бы четвертью себя (ADR 0007).
