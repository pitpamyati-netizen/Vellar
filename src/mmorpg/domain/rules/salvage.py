"""Что кузница делает с вещью, кроме починки: разбирает и перековывает.

Две работы, и обе про одно - что снаряжение не тупик. До сих пор вещь, которая
больше не нужна, знала одну дорогу: продаться торговцу за пятую часть цены. А
сырьё знало ровно одну дорогу к себе - жилу в локации.
...
- **Разбор** (``salvage``) возвращает вещь в сырьё: род вещи решает, во что она
  ...
  выйдет. Столько же материала можно было бы купить за меньшее золото, чем даёт
  скупка, - разбирают не ради выгоды, а ради того, чего в лавке нет.
- **Перековка** (``reforge``) меняет вещи оттиск: тот же вид, та же ступень, та
  ...
  и есть ответ на «выпало не то»: не выбросить находку, а переделать её, как это
  и делают везде, где у вещи есть прибавки.
