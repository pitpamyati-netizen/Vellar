...
``content/skills.toml`` называет у каждого боевого умения строку ``effect``;
здесь сказано, как движок её исполняет. Эти двое обязаны сходиться, и
``tests/domain/test_combat.py`` падает, если содержимое называет эффект,
которого здесь нет.

Новое умение кода не требует. Новый **род** поведения требует: одна запись
здесь, и исполнитель уже знает, что с ней делать.

Состояния (``entities/statuses.py``) объявляются здесь же, списком ``inflicts``
для цели и ``holds`` для себя: умение называет, какое состояние вешает и на
сколько ходов (ADR 0023).
"""
...
def cleansed_count(spec: EffectSpec, power: float) -> int:
    ...
    У умения, которое только и делает, что снимает эффекты, силе ранга больше
    ложиться некуда. У прочих - тех, где снятие идёт довеском к удару или
    барьеру, - число написано в спецификации и от ранга не зависит.
    """
...
# не руками по той же причине, что и снаряжение: это один и тот же удар,
