...
#: виды узлов переставляются внутри их категорий, боевой состав локации
#: перекладывается, а короткие тропы ложатся заново (``domain/procgen/location.py``,
#: ADR 0032, ADR 0034). Считается по выработке, а не по часам: пока в округе есть
#: что брать, она стоит как стояла, а выбитую заселяет заново. Число невелико
#: нарочно - за одну плотную вылазку округа успевает переложиться хотя бы раз,
#: иначе перекладки не видит никто (ADR 0034). Локацию, которую сутками никто не
#: трогал, кэш забывает целиком, и её поколение откатывается к нулевому само.
REGROWTH_WAVES = 12
