...
*Что у сущности за поля.* :data:`FIELDS` - единственное описание: по нему
рисуется карточка, по нему же разбирается набранное значение.

*Годится ли запись.* :func:`problems` возвращает отказы словами. Записи с
отказами не применяются, но и не пропадают: смотритель видит и правку, и
причину, по которой она пока не работает.

*Как выглядит мир с правками.* :func:`apply` собирает новое содержимое: TOML
плюс записи. Содержимое неизменяемо, поэтому это новая сборка, а не запись
поверх старой, - оттого правку и видно сразу всем.
"""
...
#: но условие задания - это несколько фраз, а не подпись.
MAX_TEXT = 600
...
class FieldKind(StrEnum):
    ...
    CHOICE = "choice"
    LIST = "list"
    #: Поле «список чисел»: целые через запятую, «1, 2, 2, 3, 4». Ни одно поле
    #: пока такое не просит — лестница вроде ``craft_rules.rank_tiers`` попросит.
    NUMBERS = "numbers"
    ...
    #: набирают одним сообщением «a | b | c». Такие — находки сбора у ремесла.
    ROWS = "rows"
...
FIELDS: Mapping[OverlayKind, tuple[FieldSpec, ...]] = {
    ...
    OverlayKind.QUEST: (
        ...
        # Спрашивается, только пока житель не выбран (``fields_for``): имя всё равно
        # берётся у жителя, если он назван (``_quest_from``).
...
#: Порода противника и узлы поиска - разные списки, и какой из них нужен,
#: зависит от того, что задание считает. Единственное поле, чьи варианты
#: зависят от соседа.
#:
#: Для боевых заданий список не кончается породами: «убей пятерых кабанов» -
#: это не «убей пятерых зверей». Следом за породами идут поимённо все
#: противники мира, включая заведённых этой же панелью (``options``), а счёт
#: умеет и то и другое (``domain/rules/quests``).
...
def _target_options(content: GameContent, record: OverlayRecord) -> tuple[str, ...]:
    ...
    Породы идут первыми - «любое зверьё» остаётся нормальным условием; за ними
    именами идут все противники, чтобы можно было заказать именно кабанов.
    """
...
def effective(
    ...
    строка с правкой и сущность, которой в ``content/`` не было вовсе. Убранная
    сущность из содержимого пропала, поэтому её поля берутся из самой записи -
    иначе «вернуть в игру» возвращало бы пустое место.
    """
...
def apply(content: GameContent, records: Sequence[OverlayRecord]) -> GameContent:
    ...
    про который нельзя сказать, что в нём стоит.

    Порядок не декоративный. Сначала встаёт мир - города, локации и люди, - и
    только потом то, что на них ссылается: задание может быть выдано жителю,
    заведённому той же панелью, а противник поселён в новую локацию.
    """
    if not records:
        return content
...
def _apply_locations(cities: Sequence[City], records: Sequence[OverlayRecord]) -> tuple[City, ...]:
    ...
    for city in cities:
        ...
        # Пустой город - город, в который незачем идти. Каждая правка по отдельности
        # законна, а вместе они оставили бы список локаций пустым.
...
# чтобы смотритель вставил его руками и снял правку. Round-trip не самоцель:
# это заготовка, а не генератор.
