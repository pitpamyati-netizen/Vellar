...
#: Потолок роста. Сто пятьдесят, а не триста (ADR 0058): триста уровней ничего
#: не открывали сверх ста пятидесяти - они растягивали одно и то же вдвое, а
#: цена этому была та, что на любом уровне игрок стоял между двумя соседними
#: ступенями снаряжения и не чувствовал ни одной.
MAX_LEVEL = 150
#: Кривая опыта. Показатель ниже прежнего (был 1,6 на трёхстах уровнях) нарочно:
#: полоса сжалась вдвое, и прежний показатель сделал бы поздний уровень вдвое
#: дороже раннего в ходах, а не в опыте. С 1,45 путь до потолка - около пяти
#: тысяч боёв: четыре боя на первый уровень, около семидесяти на сто пятидесятый.
_CURVE_FACTOR = 50.0
...
def experience_reward(*, enemy_level: int, character_level: int, base: int = 12) -> int:
    ...
    Драться сильно ниже своего уровня - не способ фармить: на разнице в 5 уровней
    награда падает до десятой части. Прежде та же десятая часть приходилась на
    десять уровней разницы - на вдвое более короткой полосе (ADR 0058) это и есть
    та же самая разница.
    """
