...
Сырьё в Vellar больше не берут из меню. Кнопка «Собрать сырьё» на экране
ремесла давала одно и то же в городе, в бою и на постоялом дворе, и всё, что
стояло между игроком и материалами, - откат в четверть часа. Теперь сырьё лежит
там, где ему и место: в узлах локации, - а взять его можно только тем, чем его
берут (ADR 0056).

Три правила, и все три - здесь.

1. **Без инструмента сбора нет.** Пустой слот «Инструмент» - это отказ, а не
   меньшая добыча: голыми руками руду не выламывают.
2. **Род инструмента решает, что им берут.** Кирка берёт руду и обломки, серп -
   травы, нож - шкуры (``items.toml``, ``tool_types``). Узел, который называет
   своё сырьё, требует и своего инструмента; узел, который его не называет,
...
def refusal(content: GameContent, character: Character, source: str = "") -> str:
    ...
    if item is None:
        return (
            ...
            "Кирку, серп и нож продают в любой лавке."
        )
