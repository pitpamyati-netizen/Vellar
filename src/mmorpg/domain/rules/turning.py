"""Оборот: что делают на трёхсотом уровне и чем за это платят.

Уровень кончается, игра — нет. Оборот — единственное место, где растут не за
...
- **голос в счётном вопросе**. Печать — это голос, и весит он ровно столько
  Оборотов, сколько человек их совершил. Палата спрашивает — про пошлину, про
  ворота, про цену воды, — а ответ считают по тем, кто за него заплатил делом.

Заклад не возвращается и не закладывается дважды: что ушло в Оборот, записано на
...
ровно так же, как круг когда-то печатал золото.
...
#: С какого уровня совершают Оборот. Он же последний: дальше расти некуда.
MIN_LEVEL = MAX_LEVEL

#: Уровень вещи, которую Палата примет в первый Оборот, и насколько он растёт с
...
#: втором ранге, отдать не жалко, а Оборот — это про жалко.
PLEDGE_EDGE_RANK = 5

#: Сколько схваток в спуске у того, кто ни одного Оборота не совершил. Коротко
...
def asking(seals: int) -> int:
    """Уровень вещи, которую Палата примет в следующий Оборот."""
...
def refusal(character: Character) -> str:
    """Пусто, когда Оборот можно совершить, иначе — почему нельзя."""
    if character.level < MIN_LEVEL:
        return (
            f"Оборот совершают с {MIN_LEVEL} уровня. Ваш уровень: {character.level}. "
...
class Sealed:
    """Совершённый Оборот: кем персонаж стал и что он за это отдал."""
...
def may_answer(character: Character) -> bool:
    """Голос есть у того, кто заплатил за него Оборотом."""
    return character.seals > 0


def voice(character: Character) -> int:
    """Сколько весит его голос: по Печати за Оборот."""
