...
У обоих открывается панель боя, и ход ждёт того, чья очередь (ADR 0021).

Границы: в локации, помеченной ``pvp``, напасть может всякий, кто стоит на
вашем узле, и защищает от этого не согласие, а забор - нижний уровень, узкое
окно уровней и ставка, которая берётся из кармана, а не из банка.

Ставка нарочно мала: проигрыш стоит десятой доли золота на руках и ран этого
боя, и никогда не стоит уровня, вещи или задания.

Ждать никого не нужно: очередь стоит столько, сколько стоит, а тот, кто ждать
больше не хочет, отдаёт бой кнопкой «Сдаться». Таймера, наказывающего того, кто
отошёл, в игре нет.
"""
