...
class User:
    ...
    персонаже: право, которое обходится заведением второго персонажа, правом не
    является. Право из ``ADMIN_IDS`` здесь не хранится вовсе - оно читается из
    окружения каждый раз (``docs/keeper.md``).
    """
    ...
    #: право смотрителя: наказание, от которого уходят вторым персонажем, - не
    #: наказание.
    ban: Ban = field(default_factory=Ban)
    #: Сколько раз смотрители выносили этому аккаунту предупреждение. Счётчик, а
    #: не наказание: по нему видно, кому и сколько раз говорили «так не надо».
    warnings: int = 0
    #: Замолчан ли аккаунт в игровой группе. Устроен как ``ban``, но мягче: игре
    #: не мешает, только сообщения в группе бот удаляет, пока срок не выйдет.
    mute: Ban = field(default_factory=Ban)


@dataclass(frozen=True, slots=True)
class Census:
    """Игра в числах: то, что смотритель проверяет.

    Считается одним запросом на открытие экрана и нигде не хранится: счётчик,
    живущий отдельно от того, что он считает, однажды расходится с ним
    (``Claude.md``, правило 8).
    """
...
class UserRepository(Protocol):
    ...
    async def warn(self, telegram_id: int, *, delta: int = 1) -> int:
        """Изменить счётчик предупреждений и вернуть, сколько их теперь.

        Строку приходится заводить: предупреждение выносят и тому, кто ни разу
        не менял настройки. Счётчик не уходит в минус — «снять предупреждение»
        останавливается на нуле.
        """

    async def set_mute(self, telegram_id: int, mute: Ban) -> None:
        """Замолчать аккаунт в игровой группе или снять это. Пустой ``Ban`` — снять.

        Строку приходится заводить по той же причине, что у ``set_ban``.
        """
...
class PlayerFilter:
    ...
    запросом; заблокированность и гильдию досовмещает вызывающий.
    """
...
class GoldFlowRepository(Protocol):
    ...
    базе, чтобы смотритель видел срез по одному игроку (ADR 0044).

    Не источник истины: запись идёт мимо повторов (ADR 0009), строка может
    отстать, потеряться или продублироваться.
    """
...
class PrivacyRepository(Protocol):
    ...
    обходится заведением второго персонажа, списком не является. Игрок, который
    ничего из этого не трогал, открыт всем, поэтому аккаунт без строки отвечает
    «виден, не блокирует никого».
    """
...
class CharacterRepository(Protocol):
    ...
    async def spend_gold(self, character_id: int, amount: int) -> bool:
        ...
        ``save`` записывает персонажа целиком, прочитанного несколько шагов назад,
        поэтому между чтением кошелька и обратной записью другое обновление могло
        истратить те же монеты - и запись вернула бы их на место. ``False`` значит,
    ...
    async def grant_gold(self, character_id: int, amount: int) -> None:
        ...
        Вторая половина :meth:`spend_gold`: запись персонажа, прочитанного мгновением
        раньше, отменила бы всё, что он успел сделать между делом.
        """
    ...
    async def arena_opponent(self, *, level: int, window: int, exclude_id: int) -> Character | None:
        ...
        играет движок (``domain/rules/combat``). Противнику не сообщают ничего, но
        дерётся он своим оружием и своими умениями (ADR 0021).
        """
    ...
    async def purge_abandoned(self, *, before: int) -> int:
        ...
        Брошенный - это первый уровень, ноль опыта, ни одного шага обучения и давно
        не тронут. Такой персонаж занимает имя, которое кому-то нужно.
        """
...
class TradeRepository(Protocol):
    ...
    истекающее само, тихо проглотило бы и то и другое.

    ``close`` - ворота, которые делают сделку неделимой: строка меняется, только
    пока она ещё стоит, поэтому двое, ответившие на одно предложение разом, дают
    ровно одно закрытие.
    """
    ...
    async def expire(self, *, before: int, scope: str | None = None) -> tuple[TradeRecord, ...]:
        ...
        Вызывающий возвращает каждую ставку её автору; вернуть запись дважды значило
        бы выдать вещь дважды, поэтому здесь и читают, и пишут одним шагом.

        ``scope`` сужает уборку до одной группы, ``None`` убирает во всех: номер
        предложения принадлежит группе, а пять минут его жизни - нет.
        """
    ...
    async def revert(self, trade_id: int) -> TradeRecord | None:
        ...
        Те же ворота, что у ``close``: откатить можно только действительно
        закрывшуюся сделку и только один раз. Двигать товар обратно - работа
        вызывающего (``application/services/group_trade.roll_back``). Время отката
        держит журнал смотрителя, вместе с тем, кто откатил.
        """
...
class ContentOverlayRepository(Protocol):
    ...
    Единственное содержимое, которое живёт в базе, и по той же причине, что
    персонаж: потерять его нельзя. Файлы в ``content/`` остаются нетронутыми,
    поэтому любую правку можно снять (``docs/keeper.md``).
    """
...
class PartyRepository(Protocol):
    ...
    терять между заходами (ADR 0029). Приглашения висят в кэше со сроком: зов,
...
class GuildRepository(Protocol):
    ...
    не уходит в минус, даже если два старейшины нажали разом. Тем же движением
    растут деяния гильдии и вклад её людей (ADR 0076).
    """
    ...
    async def save(self, guild: Guild) -> None:
        """Записать имя, основателя и состав со званиями.

        Казну и деяния этим не трогают: они двигаются условным ``UPDATE``, а не
    ...
    async def record_deeds(self, guild_id: int, character_id: int, deeds: int) -> None:
        """Записать деяния: гильдии в общий счёт, человеку - в его вклад (ADR 0076).
...
class LocationStateCache(Protocol):
    ...
    Сама карта хранения не требует - она чистая функция от места и номера
    поколения; общим остаётся то, сколько от волны каждого узла ещё стоит и кто
    по локации ходит. Ничто из этого не источник истины: потерянный Redis
    наполняет узлы заново и забывает, кто где был (``docs/procgen.md``).
    """
