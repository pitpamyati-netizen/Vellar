...
#: Насколько порода к концу полосы тяжелее своей же прямой (ADR 0068).
#:
#: Герой растёт не по прямой: сверх уровня и розданных очков он к сотне набирает
#: ВЕХИ, а веха - это прибавка, которой на первом уровне нет ни у кого
#: (``domain/rules/milestones.py``). Прямая, выставленная против героя без вех,
#: к сто пятидесятому уровню отстаёт на целый ход: обычный бой у мага падал с
#: трёх ходов до двух, и локация снова становилась формальностью, ради чего
#: договор о цене боя и написан (``tests/domain/test_combat_balance.py``).
#:
#: Поэтому здоровье породы идёт с наклоном, повторяющим наклон вех: на первом
#: уровне прибавки нет вовсе, на сто пятидесятом её ровно столько, сколько герой
#: набрал порогами. Это не «противник по уровню игрока» (ADR 0019): у породы
...
def _drift(level: int) -> float:
    """Во сколько раз порода этого уровня тяжелее своей же прямой (ADR 0068).

    Единица на первом уровне, ``1 + HEALTH_DRIFT`` на последнем, ровная прямая
    между ними: столько же, сколько герой к этой минуте набрал вехами.
    """
    from mmorpg.domain.rules.progression import MAX_LEVEL

    share = min(max(0, level - 1), MAX_LEVEL - 1) / float(MAX_LEVEL - 1)
    return 1.0 + HEALTH_DRIFT * share
...
def generate_enemy(
    ...
    *,
    ...
    health = (
        (HEALTH_BASE + HEALTH_PER_LEVEL * level * _drift(level))
        * archetype.health
        * spread
        * factors.health
        * share
