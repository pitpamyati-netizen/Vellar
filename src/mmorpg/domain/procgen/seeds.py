...
узлов, набор категорий среди них и кривая уровней. Всю раскладку - остовное
дерево, где какой узел стоит, короткие тропы, имена - решает **поколение
округи**: ``epoch_seed``. Номер поколения считает не время, а выработка - сумма
волн, снятых со всех узлов локации (``domain/rules/nodes.location_epoch``,
ADR 0035): выработали округу - она заселяется заново и другой. Меняется и то, что
стоит *в* узлах: каждый узел считает свои волны, и отработанную волну через
несколько минут сменяет следующая (``domain/rules/nodes.py``). Третье, что
