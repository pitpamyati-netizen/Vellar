...
собирается из вида, ступени, редкости и **оттиска**.

Три числа, оттиск и одно правило.
...
- **Урон** — кости вида, у которых растёт среднее. Размах остаётся размахом
  ...
  последнем, но верхняя граница нигде не выше полутора нижних
  (``entities/dice.py``, ``MAX_SPREAD``).
- **Броня** — число, считанное из ступени, рода доспеха и того, сколько
  прикрывает место.
- **Аффиксы** — прибавки, выпавшие вещи при сборке. Сколько их, решает редкость:
  обычная не несёт ни одной, необычная одну, редкая две, легендарная три,
  реликтовая четыре, и у двух последних сверх того есть особое свойство — аффикс
  полной силы. Аффиксом бывает и характеристика, и процент: пул один, как во
  всякой игре, где вещь собирают из базы и прибавок (ADR 0059).
- **Оттиск** — какой аффикс у вещи ведущий. Он же стоит в её имени («Крепкий меч
  ярости редкой работы») и в её идентификаторе, поэтому две находки одного вида и
  одной редкости — это две **разные** вещи с разными именами, а не две
  одинаковые кнопки в списке.
...
Величина аффикса не одна на всех. Она берётся из опорной, растёт со ступенью
вещи (``_tier_factor``: половина силы на первом уровне, полная на последнем — так
ступень значит что-то и для прибавок, а не только для урона) и бросается в
границах ``AFFIX_SPREAD``. Легендарной и реликтовой вещи прибавка изредка выпадает
**великой** — в полтора раза выше своего потолка; такие называются вслух на
карточке вещи, потому что это и есть та находка, ради которой ходят к логову.

Сборка чистая и повторимая: всё, что в вещи «случайно» — какие именно аффиксы,
какой величины и великие ли они, — выведено из её же идентификатора. Один и тот
же «Крепкий меч ярости редкой работы» одинаков у всех и после любого перезапуска
(``Claude.md``, правило 8).
"""
...
#: Какая доля опорной величины достаётся аффиксу на первом уровне. Дальше растёт
#: до полной к последнему: ступень вещи должна значить что-то и для прибавок,
#: иначе легендарка первого уровня даёт ровно то же, что легендарка последнего.
AFFIX_TIER_FLOOR = 0.5

#: Во сколько раз слабее процентный аффикс, выпавший обычным, а не особым
#: свойством. Особое свойство - одно на вещь и бьёт в полную силу; остальные
#: проценты идут долей от него, иначе редкая вещь с двумя процентами обгоняла бы
#: легендарную.
ORDINARY_AFFIX_SCALE = 0.6

#: Великая прибавка: во сколько раз выше своего потолка и как часто выпадает.
#: Только легендарной и реликтовой вещи, и не чаще, чем изредка: великая прибавка
#: - это находка, о которой рассказывают, а не то, с чем выходит каждый второй.
GREAT_FACTOR = 1.5
GREAT_CHANCE = 0.12
...
def affix_pool(content: GameContent) -> tuple[Affix, ...]:
    ...
    характеристики, потом особые свойства - так же, как они объявлены в
    ``items.toml``.
    """
    stats = tuple(
        Affix(
...
def _affixes_of(
    ...
    *,
    ...
    не звучит особым свойством, она звучит крупной цифрой.
    """
...
def name_of(
    ...
    здесь не украшение: оно и есть то, чем два меча одной ступени и одной
    редкости различаются на слух.
    """
...
def catalogue(content: GameContent) -> tuple[Item, ...]:
    ...
    десятки тысяч предметов ради того, чтобы каждый второй никогда не выпал.
    """
    return tuple(
...
def roll_drop(
    ...
    *,
    ...
    две находки одного вида отличаются друг от друга: ведущий аффикс, а вместе с
    ним имя и числа (ADR 0059).
