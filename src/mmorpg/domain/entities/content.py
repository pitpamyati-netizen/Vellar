...
class Skill:
    ...
    fork: str = ""
    #: Состояние, на которое это умение отвечает: удар по цели, на которой оно
...
class GuildTier:
    ...
    сколько человек в неё помещается, ``store_slots`` — сколько разных вещей
    держит её хранилище, ``exp_percent`` и ``gold_percent`` — что ступень
    добавляет своим за выигранный бой с миром.
    ...
    #: Сколько разных вещей помещается в хранилище гильдии на этой ступени
    #: (ADR 0077). Место считается видами, а не штуками: стопка стрел занимает
    #: столько же, сколько один меч, - как и в сумке игрока.
    store_slots: int = 0
...
class Subclass:
    ...
    ``power``      уклад ветки: правило, которым она меняет бой
                   (``rules/subclass_powers``). Это и есть ответ на «чем эта
                   ветка играется иначе»: прибавки говорят «насколько», уклад -
                   «как»;
    ...
    level: int = 1
    skill_code: str = ""
    #: Уклад ветки: правило, которым она меняет сам бой
    #: (``rules/subclass_powers``). Пусто - ветка правил не меняет.
    power: str = ""
