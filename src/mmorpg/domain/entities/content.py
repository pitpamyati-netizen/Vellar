...
class Skill:
    ...
    #: одном уровне и в одной ветви, а изучить из них можно только одно: взяв
    #: первое, второе закрываешь, пока не разберёшь взятое у наставника
    #: (ADR 0024). Пусто - умение ни с чем не спорит.
    fork: str = ""
...
class ArmorType:
    ...
    ``armor`` - доля брони относительно кожаного доспеха того же уровня. Именно
...
class GearTier:
    ...
    Ступеней семнадцать на все сто пятьдесят уровней, и вещь бывает только на ступени —
...
class GearArchetype:
    ...
    Сами вещи в ``content/`` не пишут. Видов под сотню, ступеней под два десятка,
    редкостей пять — тысячи вещей; написанные руками, они были бы файлом, который
    никто не правит. Пишется вид, а вещь собирается из вида, ступени и редкости,
    как противник собирается из породы и уровня (``domain/procgen/items.py``).
    """
...
class Rarity:
    ...
    #: снаряжения считается от уровня вещи, а редкость её только множит: доброй
    #: ковки меч ходит дольше ветхого не потому, что бьёт сильнее (ADR 0057).
    toughness: float = 1.0
...
@dataclass(frozen=True, slots=True)
class Turning:
    """Голосование: вопрос, который Большой совет выносит игре, и ответы, между
    которыми считают голоса.
...
class ProgressionRules:
    ...
    #: Сколько каждая из семи характеристик растёт сама по себе за уровень. Прежде
    #: не росла ни одна: розданных очков хватало на две, и остальные пять стояли
    #: на пятёрке и на трёхсотом уровне - крит и уклонение у воина были ровно те
    #: же, что при создании (ADR 0058). Рост общий для всех классов; чем персонаж
    #: отличается от соседа, по-прежнему решают розданные очки.
    ...
    #: Чего стоит каждый ранг, с первого по последний. Прежде очко стоило одно и
    #: то же везде, и шестьдесят умений по пять рангов сходились ровно с тремя
    #: сотнями уровней: к трёхсотому было изучено всё, и два воина одного уровня
    #: были неразличимы. Теперь дерево дороже дохода, и «что взять» - вопрос
    #: (ADR 0024).
