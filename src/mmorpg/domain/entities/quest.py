...
class Quest:
    ...
    location_slot: int = 0
    #: Ветка специализации, испытанием которой это задание является (ADR 0074).
    #: Пусто - обычная городская работа. У испытания нет города: его выдаёт
    #: наставник, а не доска, и поэтому на доске оно никогда не появляется.
    trial_for: str = ""

    @property
    def is_trial(self) -> bool:
        return bool(self.trial_for)
