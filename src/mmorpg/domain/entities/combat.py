...
сущность с разным составом сторон (ADR 0021).

:class:`Combatant` - это боец, и разница между героем и волком в нём одна: за
героем стоит персонаж, а за волком - порода. Разница между живым игроком и
слепком тоже одна: у живого ход ждёт нажатия, за слепком ходит движок.
...
class EventKind(StrEnum):
    ...
    CRIT = "crit"
    HEAL = "heal"
    #: Барьер: сколько чужого урона он ещё удержит (``StatusKind.BARRIER``).
    BARRIER = "barrier"
    ...
    #: круга (ADR 0065).
    JOINED = "joined"
    #: Связка сошлась: удар пришёлся по цели, на которой держится то, на что это
    #: умение отвечает (``rules/combo``).
    COMBO = "combo"
    #: На стороне бойца встал приведённый им: поднятый павший или спутник
...
class Combatant:
    ...
    evade_charges: int = 0
    #: Уклады изученных пассивных умений с их величинами
    #: (``rules/passives``). Считаются при сборке бойца и весь бой не меняются:
    #: пассивку не снимают посреди боя.
    powers: Mapping[str, float] = field(default_factory=dict)
    #: Уклады взятых веток специализации (``rules/subclass_powers``) - правила,
    #: которыми ветка меняет сам бой.
    ways: frozenset[str] = frozenset()
    #: Чей это слуга или спутник. Ноль - боец сам по себе. Приведённый в бой
    #: уходит с поля вместе с тем, кто его привёл: некому держать - некому и
    #: драться.
    master_id: int = 0
