...
Состояние - это эффект с именем из этого списка (ADR 0023). Имя решает всё
остальное: беда это или благо, что оно прибавляет, точит ли цель каждый ход и
каким родом урона, и пропускает ли носитель ход. Два умения, вешающие горение,
вешают одно и то же горение - оно обновляется, а не складывается вдвое
(``EffectStack``).
...
У состояния бывает и то, что от величины не зависит вовсе
(``flat_modifiers``): защита прибавляет брони столько, сколько стоит закрыться
на этом уровне, а уклонения - всегда одну и ту же треть. Обе прибавки
объявлены здесь, потому что обещает их состояние, а не тот, кто его повесил.
"""
