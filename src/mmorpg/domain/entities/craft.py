...
class QualityTier:
    ...
    (ADR 0060). Так работает всякое ремесло, где кузнец отличается от подмастерья
    не числом молотков, а тем, что у него из-под рук выходит.

    Тому, у чего редкости нет вовсе - зельям, точильным камням, сырью, -
    качество платит по-старому: ``extra`` вещей сверху. А ``refund_percent`` -
    сырьё, оставшееся неистраченным, и оно одинаково для всего.
    """
