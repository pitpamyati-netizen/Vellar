...
class GeneratedLocation:
    ...
    Скелет - число узлов, их места, имена, уровни и главные тропы - у Лугов один
    и тот же навсегда, и игрок, выучивший дорогу на слух, продолжает её знать
    (ADR 0013). ``epoch`` - поколение округи: оно переставляет конкретные виды
    узлов внутри их категорий, перекладывает боевой состав локации и заново
    стелет короткие тропы, а двигает его выработка, не время
    (``domain/rules/nodes.location_epoch``, ADR 0032, ADR 0034).
    Меняется и то, что стоит в узлах (``domain/rules/nodes.py``).
    """
