...
class Character:
    ...
    house_id: str = ""
    # Взятые ветки специализации (``domain/rules/subclass.py``, ADR 0074). Дерево
    # ветвится трижды - на 30, 75 и 150 уровне, - и взятое складывается:
    # Иллюзионист - это Оккультист, ставший Псиоником, ставший Иллюзионистом, а
    # не кто-то третий. Порядок в списке значения не имеет: свёрток прибавок
    # складывается, а не накапливается.
