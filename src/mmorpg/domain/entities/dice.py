...
Но размах — это характер, а не лотерея. Поэтому у оружия есть ещё и прибавка:
часть удара, которую не бросают вовсе («1d10+3»). Она растёт вместе с костями,
и потому доля брошенного в ударе одна и та же на первом уровне и на трёхсотом.
Без неё булава обещала «от 9 до 630», и решать по такому числу было нечего
(``content/items.toml``, ``[meta].weapon_types``).
