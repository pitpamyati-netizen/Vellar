...
class Settings(BaseSettings):
    ...
    # жалоба игрока, а насыщенный день на сотню игроков - несколько мегабайт.
    ...
    # принимают либо числовой id чата (-100...), либо @username. Пусто - значит,
    # этой части просто нет: локальный запуск без канала обязан играться.
    channel_id: str = ""
    # ``*`` значит «в любой группе, куда бота добавили». Настройка для проб, а не для
    # боя: групповую половину игры можно попробовать, не разыскивая числовой id.
    group_id: str = ""
    ...
    # его себе изнутри игры нельзя (``docs/keeper.md``).
    admin_ids: str = ""

    world_seed: str = "vellar-prime"
    # Мир не переворачивается по часам: локация держит свою карту, пока её не
    ...
    # держит живыми истоки корутин. Это инструмент разработки, поэтому настройка
    # оставлена пустой: перила ценой в пропускную способность просят, а не наследуют.
    ...
    # Сколько отправок в секунду бот себе позволяет. Telegram считает около тридцати
    # и на бота целиком; за перебор отвечают «подожди».
    ...
    def uses_redis(self) -> bool:
        ...
        (``Claude.md``, правило 8). Один процесс удержит их сам, а это на одну службу
        меньше.
        """
