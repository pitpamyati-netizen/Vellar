...
дороге в Telegram, делается заново, когда связь вернулась.

Повторяется только пропавшая связь. Всё, на что Telegram ответил, - неверный
запрос, игрок, заблокировавший бота, неверный токен - это ответ, а не обрыв.

Цена честная: запрос, дошедший до Telegram и потерявший только ответ,
отправляется дважды, поэтому игрок может увидеть один экран дважды. Для
незрячего повторённый экран - шум, а тишина - тупик.

``getUpdates`` нарочно оставлен в покое: у цикла опроса aiogram есть
собственное отступление вокруг него.
"""
