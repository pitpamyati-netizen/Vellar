...
Проверка стоит здесь, а не в каждом хендлере: забыть её в одном месте значит
оставить дверь приоткрытой. Заблокированный не проходит никуда - ни в игру, ни
в бой, ни в группу.

Аккаунт читается один раз и кладётся в данные под именем ``user``, поэтому
хендлеру не приходится читать его второй раз.

В личке заблокированному отвечают - человек должен знать, что случилось и до
каких пор. В группе молчат: бот там отвечает только на обращённое к нему.

Мьют мягче: замолчавший играет как обычно везде, кроме игровой группы, где бот
удаляет сказанное им. Ничего ему при этом не отвечают.

Здесь же стоп-кран: пока в кэше висит флаг режима обслуживания (ADR 0045), бот
отвечает всем, кроме смотрителей, одной строкой и не пускает дальше.
"""
