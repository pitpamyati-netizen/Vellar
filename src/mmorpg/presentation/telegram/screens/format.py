...
#: Предел самого Telegram. Экраны до него не доходят и не должны: своё «около
#: девятисот» держит ``tests/presentation/test_accessibility.py``. Он здесь
#: затем, что отправка обязана быть последней преградой, а не первой потерей —
#: раньше отправлялась только первая страница, и хвост длинного списка умений
#: пропадал молча, без единого слова о том, что он был.
HARD_LIMIT = 4096
