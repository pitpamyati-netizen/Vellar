...
Рисуется только для тех, чей Telegram id стоит в ``ADMIN_IDS``, и говорит об
этом первой строкой.

Правил здесь два, и оба взяты у остальной игры. Первое: каждое действие
отвечает числом, которое получилось, - смотритель сверяет игру, а не
выслушивает заверения. Второе: доступность обычная - служебный ряд на месте,
псевдографики нет, значение поля читается словом («не заполнено»), а не
пустотой.

Карточка сущности рисуется по описанию полей из ``domain/rules/overlay.py``, а
не по списку кнопок: новое поле у задания должно появляться на экране само.
"""
...
#: строке. Строку карточки слушают, и лучше услышать пять полных строк, чем
#: восемь оборванных; полное значение всегда есть на экране поля.
CARD_FIELDS = 5
CARD_VALUE = 80
...
def filtered_listing(
    ...
    Нумерация строк идёт по *отфильтрованному* списку, поэтому фильтр применяется
    в одном месте, а не в каждом обработчике.
    """
...
def entity_screen(
    ...
    четырнадцать, и все в одно сообщение не поместились бы. Строка говорит, что
    стоит в поле, кнопка под ней его меняет.
    """
    single = overlay_rules.TITLES[record.kind][0]
...
def _short(content: GameContent, spec: FieldSpec, record: OverlayRecord) -> str:
    ...
    Карточка читается вслух целиком, и четырнадцать полей по шестьсот знаков -
    это не карточка. Полное значение всегда на один шаг ниже, на экране самого
    поля.
    """
...
def field_screen(
    ...
    # Значение читается вслух и потому укорочено: у поля-таблицы в нём может лежать
    # пять находок с полусотней биомов (правило доступности 4). Строки целиком
    # стоят кнопками ниже.
...
def player_screen(
    ...
    из настройки: остальные не видят ни строки об этом, ни кнопки.
    """
    city = content.city(player.city_id).name if content.has_city(player.city_id) else player.city_id
    lines = [
...
def ban_screen(player: Character, view: KeeperView, reason: str = "", notice: str = "") -> Screen:
    ...
    Причина набирается до срока, потому что читает её тот, кого заблокировали:
    сказать «за что» - часть наказания, а не украшение.
    """
    lines = [
...
def trades_screen(player: Character, view: KeeperView, notice: str = "") -> Screen:
    ...
    плательщику приходит ровно то, что получил продавец.
    """
...
def keeper_guild_screen(
    ...
    *,
    ...
    Основателя не трогают - ни звание, ни вывод: гильдию без него распускают.
    Казну двигает хендлер условным ``UPDATE``.

    Номер участника - сквозной по всему составу, а не по странице: двенадцатый
    остаётся двенадцатым на любой странице (``Claude.md``, правило 9), и по этому
    же номеру его находит автомат.
    """
...
#: Сколько записей журнала помещается в одно сообщение. Панель показывает
#: последнее, а не хранит историю за всё время.
LOG_SHOWN = 14
...
def log_screen(view: KeeperView, page: PageState | None = None, notice: str = "") -> Screen:
    ...
    Журнал существует потому, что смотрителей больше одного, а панель раздаёт
    золото, уровни и блокировки: работа, которую нельзя посмотреть, - это работа,
    за которую некому отвечать. С карточки игрока журнал сужается до него одного
    (``log_target``).
    """
