...
служебные объявления. Это не лента того, что делали игроки. Правила о словах
живут в ``Narrative.md`` («Канал»); те, что можно закрепить кодом, - здесь:

- заголовок - это всё сообщение для того, кто остановился после первой строки;
- только чистый текст, ``parse_mode=None``: канал читают и экранные дикторы
  (правило доступности 14);
- никакой псевдографики, числа вызывающий пишет как ``X из Y`` (правило 5);
- не больше одного значка, в начале строки, выбранного по смыслу (правило 6);
- жёсткий предел длины: пост в канале на страницы не режется;
- слова игрока, а не команды, которая игру выпускает: пост, называющий модуль,
  коммит или базу, отвергается на отрисовке.
