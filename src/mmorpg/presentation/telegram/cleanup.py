...
Группа принадлежит тем, кто в ней разговаривает, и бот, оставляющий каждый
свой ответ на месте, превращает разговор в протокол: прокрутить его назад на
слух стоит настоящего времени. Поэтому сообщение в группе временное
(``Narrative.md``, раздел 9).

**Только в группе.** В личной переписке не удаляется ничто: там сообщение *и
есть* экран (``docs/accessibility.md``, правило 3).

Удаление - фоновая задача, результата которой никто не ждёт. Трогать
обновление, её породившее, ей нельзя ни при каких обстоятельствах: удалённое
модератором сообщение, отобранное право администратора, сбой сети - всё это
тихо отбрасывается.
"""
