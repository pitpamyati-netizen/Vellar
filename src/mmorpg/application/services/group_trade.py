...
Хендлер разбирает сообщение и печатает фразу; всё между этими двумя делами
происходит здесь (``Claude.md``, правило 5). Сервис знает хранилища и правила
домена и ничего не знает о Telegram.

Рядом живут два разных вида действий:

- **передача** случается сразу: получателю она ничего не стоит, и просить у
  него подтверждения подарка значило бы добавить лишний шаг;
- **предложение** чего-то стоит обеим сторонам, поэтому оно объявляется,
  получает номер и пять минут ждёт ответа.

Предложение держит сторону автора в эскроу: объявленная продажа вынимает вещь
из сумки продавца, объявленная покупка - золото из кошелька покупателя. И то и
другое возвращается, когда предложение отклонили или оно вышло по сроку. Тот,
кому предложили, не ставит ничего, пока не ответил.

Отсюда три следствия:

- предложение не сорвётся оттого, что *автор* тем временем истратил свою
  сторону: тратить ему уже нечего;
- эскроу переживает перезапуск, потому что живёт в журнале сделок в PostgreSQL,
  а не в кэше со сроком;
- каждая закрытая сделка платит пошлину, и это единственное место, где золото
  уходит из игры (``domain/rules/economy.trade_tax``).

То, *чем* отвечает вторая сторона, обязано быть у неё в ту самую минуту, когда
она отвечает. Поэтому каждый кошелёк здесь двигается через
``spend_gold``/``grant_gold`` - одним условным шагом в базе - и никогда записью
персонажа, прочитанного несколько ``await`` назад.
"""
...
async def return_stake(
    ...
    Отдельная функция, а не метод: ставку возвращают и групповым сервисом на ходу
    игры, и :func:`release_expired_offers` на её старте.
    """
...
async def release_expired_offers(
    *,
    ...
    Уборка перед каждой командой в группе (``GroupTrade._sweep``) хватает, пока
    кто-то говорит. Предложение, объявленное в группе, которая потом затихла,
    держало бы вещь автора неизвестно сколько, поэтому игра убирает ещё и один
    раз на старте.

    Это не таймер (``Claude.md``, правило 3): ничего не откладывается и ничто не
    просыпается - это случается там, где игра и так останавливается.
    """
...
async def roll_back(
    ...
    *,
    ...
    Вещь и золото возвращаются порознь нарочно: откат нужен там, где обманувший
    успел и вещь надеть, и золото потратить, а «всё или ничего» чаще всего
    означало бы ничего. Возвращается то, что есть, а чего нет - названо числом.
...
class GroupTrade:
    ...
    async def _settle(self, record: TradeRecord, *, now: int) -> GroupOutcome:
        ...
        # может тратить в это самое мгновение. Предложивший покупку заплатил при
        # объявлении, и его золото уже в эскроу.
    ...
    async def _sweep(self, now: int) -> None:
        """Вернуть ставки всех просроченных предложений, где бы они ни были.

        Выполняется перед каждой командой, а не по таймеру: предложения делают только
        в группе, поэтому просроченное обнаруживается, когда кто-то заговорит.

        Нарочно не ограничено ``self.scope``: кто заговорил следующим, тот и убрал за
        всех. То, что иначе держала бы затихшая группа, подберёт
        :func:`release_expired_offers` при следующем запуске. Отсрочка
        (``SWEEP_GRACE_SECONDS``) делает «просрочено» ответом вместо «не найдено».
        """
