...
# импортом NodeKind: содержимое говорит на языке содержимого.
...
def _parse_classes(
    ...
    for entry in raw.get("class", ()):
        ...
        # Класс, который не умеет носить ничего, дерётся голыми руками и в рубахе.
        # Такого в игре нет, и молчаливой опечаткой он тоже быть не должен.
        if not weapon_types:
...
def _validate_skill_weapons(
    ...
    двуручник. Проверка дешёвая, а ошибка тихая, поэтому она здесь.
    """
...
#: реликтовую вещь. Реликтовое не устаревает никогда: платить за него надо
#: либо логовом, либо длинной дорогой.
...
def _parse_rarities(meta: Mapping[str, Any], problems: list[str]) -> tuple[Rarity, ...]:
    ...
    for entry in meta.get("rarities", ()):
        ...
        # кнопка в списке - это её текст (правила доступности 6).
...
def _parse_items(
    ...
    for entry in meta.get("weapon_types", ()):
        ...
        # Размах читается как объявлен, но выше потолка не пускается: «в полтора раза» -
        # правило игры, а не пожелание содержимому. Объявленное сверх него - ошибка
        # автора, и она называется вслух.
    ...
    for entry in raw.get("item", ()):
        ...
        if kind_raw not in {kind.value for kind in ItemKind}:
            ...
            continue
        # Снаряжение руками не пишут: оно собирается из вида, ступени и редкости, а
        # написанная вещь молча не имела бы ни урона, ни брони, ни характеристик.
...
def _parse_quests(
    ...
    вещь, которой оно платит, и задание, за которым оно идёт.
    """
    ...
    for rank in range(1, max(1, rules.max_rank) + 1):
        for entry in lines:
            ...
            if output_id not in item_ids:
                ...
                continue
            # Лестница расходников и делаемого сырья меняет ступень раз в два ранга, а
            # снаряжение - каждый ранг: одно и то же зелье на двух соседних рангах - это
            # две одинаковые кнопки (правила доступности 5-7).
            ...
            recipes.append(
                Recipe(
                    ...
                    # Работа растёт вместе с рангом, потому что растёт и его цена: иначе поздние
                    # ранги брались бы теми же партиями, только их было бы вдесятеро больше.
...
def _validate_tools(crafts: Sequence[Craft], gear: ItemContent, problems: list[str]) -> None:
    ...
    это ремесло, в котором нельзя работать.
    """
