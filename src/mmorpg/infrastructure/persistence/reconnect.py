...
Пул asyncpg переподключается сам, но запрос заново не выполняет, поэтому
PostgreSQL, перезапустившийся между двумя обновлениями, стоил игроку действия.

:class:`ReconnectingPool` стоит между хранилищами и пулом и этот вызов
повторяет. Черта проведена там, где кончается уверенность
(``docs/adr/0009-repeating-a-lost-query.md``):

- **соединение не было получено** - не отправлено ничего, PostgreSQL о запросе
  не слышал, поэтому повторить можно что угодно, запись в том числе;
- **соединение получено, и запрос упал на нём** - ``SELECT`` выполняется
  заново, потому что прочитать дважды значит прочитать то же самое, а
  ``UPDATE`` - нет: сервер мог его закрепить и потерять только ответ, и повтор
  забрал бы золото дважды.

Пул обёрнут, а не унаследован: хранилища зовут ``fetch``, ``fetchrow``,
``fetchval`` и ``execute`` ровно как раньше.
"""
