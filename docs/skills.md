...
С уровнем это не растёт. Боевой экран первого уровня и последнего несёт те же номера в
...
**Продвижение ровное.** Сорок открытий на сто пятьдесят уровней — одно на три-четыре, и
нигде по дороге нет долгого ожидания следующей кнопки. Уровни открытия живут
...
Ранг, на котором открывается грань, — одно число на всю игру, из `skills.toml`
(`edge_rank_for`). Прежде Печать Палаты сдвигала его раньше; Печати больше нет,
новое имя грани не трогает (ADR 0048).
...
очков — **480 очков**, а сто пятьдесят уровней по два очка дают **298**: к пределу
закрыто около трёх пятых дерева. Раньше числа сходились ровно, и два воина на потолке
были неразличимы побайтово (ADR 0024, ADR 0058, `tests/domain/test_skill_tree.py`).
...
способ боя — одно и то же, а не два списка рядом, и у этого есть последствие в бою:
сборка в одной ветви чаще повторяет тег и держит разгон, а собранная вразброс — чаще
ломает размен разнобоем (`combat`, ADR 0021).
...
и это и есть выбор. Экран умений печатает три числа: «Вложено по ветвям: Напор 12,
Заслон 3, Финт 0».
...
последнем, а содержимое не приходится перенастраивать по мере роста полосы: игрок
...
- **лечение по площади** (`EffectCategory.HEAL` с `aoe`) лечит весь отряд, и доля
  ...
  запаса; грань «снять беды» на таком умении снимает их каждому. Прибавки тоже
  делятся по-честному: `healing_done_percent` — того, кто лечит, а
  `healing_taken_percent` — того, кого лечат, и считается он у каждого свой
  (`combat._heal`).
...
Тот, чей ход не настал, видит то же состояние с кнопками `[Что там в бою]`,
`[Разбор боя]` и `[Сдаться]` вместо панели.

Строка следа на панели — одна: совет, а не абзац. `[Разбор боя]` (`/разбор`)
открывает отдельный экран `breakdown_screen` — намерение каждого врага и что оно
сулит, след и разгон целиком, что делают три стойки, прозвища врагов. Это не
ход: экран другой, счётчик стоит, возврат — `[Что там в бою]`. Механику разбор
не меняет ничем — это тот же текст, собранный в одном месте, а не размазанный по
каждой строке каждого хода (ADR 0050).
...
**напор**, **заслон**, **финт**. Круга взаимных контр («тег X бьёт тег Y») больше нет
(ADR 0050): это три разные вещи, и ни одна не «бьёт» другую.
...
- **Намерение и брешь.** Тот, за кого ходит движок, объявляет свою стойку *до* хода
  ...
  раны (у обычного противника ниже четверти — заслон) и почти павшая цель (в напор,
  добивать); эпик и босс не встают в заслон и раненые не цепенеют. Стойка одинаково
  открывает своего и чужого: кто объявил **напор** — тот **breached** (удар по нему
  мимо брони, его собственный ответ вполсилы); **заслон** бьёт вполовину и множит свою
  броню на 1,5; **финт**, объявленный породой, не уклоняется.
...
- **Разнобой.** Три *разных* тега подряд — противник, на ком размен сломался, теряет
  ближайший ход, а след тратится. Это не контра, а награда за непредсказуемость.

Тег умения читается с его эффекта (`tag_of`): удар — это напор; удар прицельный — в
броню, в слабое место, в метку — или оставляющий цель в помехах — это финт; всё,
что лечит и укрывает, — заслон. Содержимое вправе назвать тег прямо через
`tag = "precision"` в `skills.toml`.

**Каждый класс дотягивается до всех трёх тегов.** Это проверяет загрузчик
(`_check_branches`): у класса должны быть и напор, и заслон, и финт, иначе он отрезан
от целой линии игры. Обычная атака — всегда напор, поэтому каждому классу нужны свой
заслон и свой финт, и обе есть к четырнадцатому уровню — вот что покупают перекрытия
`tag` в содержимом.

**Удар из незаметности не уклоняется** (`EffectSpec.always_hits`). Умение может
потребовать незаметности (`requires_stealth` в `skills.toml`, как `weapons`) — без неё
это отказ до хода, ход не тратится (ADR 0050).
...
- Порядок разрешения хода: сначала намерения и тег игрока, потом действие игрока, потом
  каждый живой противник — если только его не сбил с ритма разнобой, — потом
