...
- [ ] У этой версии есть раздел в `content/changelog.toml`, написанный как то, что игрок
      ...
      сам скрипт: `latest` — это «то, что в игре сейчас», и он отказывается публиковать
      файл, отставший от `src/` и `content/`, называя коммиты, о которых не рассказали.
      Назвать версию числом можно всегда — тогда выбор сделали вы
