...
- [ ] Новые ключи прибавок сначала добавлены в `traits.toml [meta].modifier_keys`, а
      ключ, который называет умение, есть в `modifiers.EFFECTIVE_KEYS` —
      движок читает этот список и больше ничего (ADR 0018)
- [ ] Таблица мира по-прежнему покрывает уровни 1–150 без пробелов
...
- [ ] Интеграционные тесты проходят. Это *единственное* место, где выполняется SQL:
      ...
      PostgreSQL отвергнет, ни зарезервированного имени
...
- [ ] У этой версии есть раздел в `content/changelog.toml`, написанный как то, что игрок
      теперь может сделать, и `--changelog latest --dry-run` его рисует. Это проверяет и
      сам скрипт: `latest` отказывается публиковать файл, отставший от `src/` и
      `content/`, называя коммиты, о которых не рассказали
