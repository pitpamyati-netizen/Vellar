...
встаёт там, где стояла засада; тайник — где были следы), перекладывает боевой
состав локации (сколько узлов встретит стаю, сколько — сильного одиночку) и
заново стелет короткие тропы. Категория узла, его имя и уровень при этом не
трогаются. Состав **находок** («сбор, сбор, тайник») постоянен и берётся со
скелета — на него завязаны `search`-задания; боевой состав перекладывает
поколение, но одна стая в локации есть всегда
(`docs/adr/0034-locations-bigger-livelier-fewer-elites.md`). Номер поколения
считает **выработка, а не время**: `sum(node.wave) // REGROWTH_WAVES` (12), то
есть округа перекладывается, когда её как следует проработали, и оседает к
исходной, когда её надолго оставили
...
- **Категория** внутреннего узла взвешена и постоянна: бой 45, находка 46, святилище 9.
  Вид находки решает скелет: сбор (15), событие (15) или тайник (14). Вид боевого
  узла решает поколение округи — стычка (37) или сильный одиночка (6), — но одна
  стая в локации есть всегда.
- **Хвост локации закреплён: подступ, а за ним босс.** Перед боссом стоит
  `approach_length(count) = 1 + (count − 16) // 8` узлов-стражей (один-два, по
...
- подступ — вереница из 1–2 эпических боёв, босс висит только за последним стражем, и
  выбей подступ с боссом из графа — к боссу пути нет, а к выходу есть;
- скелет — имена, уровни, число узлов, категории, состав находок — переживает смену
  поколения; босс остаётся на месте; боевой состав по поколениям меняется, но стая
  есть всегда; `search`-задание на вид узла выполнимо в любом поколении.
