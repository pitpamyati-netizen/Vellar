...
замыслу — сорок четыре умения на сто пятьдесят уровней, боевое примерно раз в семь уровней и
...
выпускает только то, что уже есть в `items.toml`: ступень снаряжения берётся из
`[meta].rank_tiers`, ступень сырья — из лестницы. Стоить сделанное обязано больше
своего сырья на каждом ранге: работа, платящая меньше, чем продажа сырья, — ловушка,
и `tests/content/test_crafts_content.py` её не пропускает. Штучное, у чего лестницы
нет, по-прежнему пишется как `[[recipe]]` с прямыми `inputs` и `output`.

Ранги, размеры сбора и три ступени качества живут в `crafts.toml [meta]`; читают их
`domain/rules/crafts.py`. У нового собирающего ремесла обязан быть свой инструмент —
запись в `items.toml [meta].tool_types` с `craft = "fishing"` и своим `sources`, и
`[[gear]]` под неё: без инструмента сырьё не берут вовсе, и загрузчик такое ремесло не
пропустит (ADR 0056). См. `docs/crafts.md`.
...
Снаряжение **руками не пишут**. Меч существует на каждой ступени в пяти
редкостях, и так же — любой другой вид: тысячи вещей. Пишут *вид*, а вещь
собирается из вида, ступени и редкости — так же, как противник собирается из породы и
уровня (ADR 0015).

Ступеней семнадцать, и они привязаны к городам: четыре ранние в стартовом
городе, дальше по одной на входе почти в каждый следующий (`world.toml`,
`level_min`). Пересёк границу города — в лавке и добыче новая ступень: новое
прилагательное и числа выше (ADR 0052). Новую ступень добавляют в
`[meta].tiers` строкой с прилагательным в четырёх родах; загрузчик держит первую
на уровне 1 и не пускает две на один уровень.
...
Сырьё объявляет `source` — какого оно рода: `руда`, `травы`, `волокно`, `шкуры`,
`рыба` или `древесина`. Делаемое сырьё — слиток, кожа, ткань, брус — `source` не
несёт вовсе: его не берут в земле, его выделывают (ADR 0062).
Тем же словом инструмент говорит, что он берёт (`tool_types.sources`): киркой не
срезают траву. Названная жила отдаёт только свой род, поэтому травяная делянка платит
травами, а рудная жила рудой (`domain/rules/adventure.GATHER_SOURCES`); безымянная
отдаёт то, что берёт инструмент в руках. Сырьё без `source` выдавал бы
