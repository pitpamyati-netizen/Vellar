...
| Где | Что |
| --- | --- |
| PostgreSQL | пользователи (`users`, `blocks`), персонажи (сырые характеристики, уровень, опыт, золото, золото в сундуке, залог арены, износ надетого, дом, уходы), сумка (`inventory`), снаряжение, набор умений с рангами и гранями, выбранные черты, город, ход заданий и ремёсел, настройки доступности, сид мира, сделки (`trades`: стоящий эскроу и журнал закрытых), отряды (`parties`, `party_members`) и гильдии (`guilds`, `guild_members`), правки смотрителя (`content_overlay`), его журнал (`keeper_log`) и срез движений золота (`gold_flow`) |
| Redis | состояние автомата (нарочно без срока) и со сроком: текущий экран, начатый бой и кто в нём, зовы в отряд и гильдию, общее состояние каждой локации, кто в ней стоит и осело ли в ней блуждающее подземелье, надбавка сводки за переворот, служебные флаги смотрителя, отсев повторных обновлений, кэш прилавка лавки |
...
| `guild-call:{character}` | неотвеченный зов в гильдию | пять минут |
| `loc:{city}:{slot}:roamer` | блуждающее подземелье, осевшее в локации | окно появления |
| `loc:{city}:{slot}:roamer:held` | кто сейчас внутри него (замок `SET NX`) | заход |
| `digest:{character}:{rotation}` | надбавка сводки уже взята | до переворота |
| `keeper:maintenance` | режим обслуживания (ADR 0045) | срок объявления |
| `keeper:reset:{user}` | сбросить экран этого игрока | до нажатия |
...
Кривая опыта считается один раз при импорте для уровней 1–150, поэтому найти уровень
