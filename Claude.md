...
- `domain/` — чистая логика, только stdlib, без async и I/O.
  ...
  `skills.toml`), `skills` (изучение, ранги, грани, слоты), `quests` (счёт и
...
9. **Новый экран** добавляется в `tests/presentation/conftest.py::all_screens` —
   ...
   нигде. Ряд под страницами показывает только то, что работает и куда-то ведёт. **Группа — не экран**: там нет служебного ряда и нет
