...
- `domain/` — чистая логика, только stdlib, без async и I/O.
  ...
  спецификация), `edges` (что грань меняет в умении: словарь объявляется в
...
постоянное умение работает изученным).
...
7. **Контент — в TOML, не в коде.** Текст, обещающий то, чего движок не считает, —
   ...
   редкости, а не пишется у каждой вещи (ADR 0015).
   ...
   **Модификатор, которого движок не читает, — не прибавка, а обещание**:
   `physical_damage_percent` и `magic_damage_percent` не считает никто, урон
   называется `damage_percent`. Единственное
