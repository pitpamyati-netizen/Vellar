...
`items.toml` (в `[meta]` — рода оружия и доспеха, доля брони по слотам),
...
есть род, и род решает броню, допуск класса и требование умения).

**`tests/`** — `domain/` (слои держит `test_layering.py`, длину боя —
`test_combat_balance.py`, снаряжение — `test_equipment.py`), `content/`, `presentation/` (доступность, канал,
...
6. **Раса, класс и ремесло — три разные оси.** Раса — кто персонаж такой,
   ...
   Чем он дерётся — четвёртая ось: **род снаряжения это допуск** (ADR 0014).
   Класс носит своё (`classes.toml`: `weapons`, `armor`), умение работает своим
   (`skills.toml`: `weapons`), и список умения бывает только уже списка класса —
   иначе это кнопка, которая не сработает никогда. Отказ приходит словами, а
   кнопки «Надеть» у чужой вещи нет вовсе.
7. **Контент — в TOML, не в коде.** Текст, обещающий то, чего движок не считает, —
   ...
   Сила умения — всегда процент: урон от
   стандартного удара, лечение и щит от максимума здоровья, усиление от самого
   модификатора. Абсолютных чисел в контенте нет (ADR 0007) — броня доспеха тоже
   считается из уровня вещи и долей её рода, а не пишется числом (ADR 0014).
   **Описания у вещи нет**: вещи выпадают сотнями, и фраза у каждой была бы
   выдумкой на месте — что вещь такое, отвечают её род, слот и числа.
